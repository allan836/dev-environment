...
provision.sh                       ← root of dev-environment repo
    │
    ├── Installs Vagrant + hypervisor on host
    │
    └── Ubuntu 24.04 VM
            │
            ├── cloud-init         ← first-boot OS config
            │
            └── Ansible playbook   ← installs all developer tools
                    │
...
only interacts with the VM via `vagrant ssh`. The host laptop is untouched.
...
| Language runtimes | Host or VM | Node/nvm, Python/pyenv, Java/SDKMAN, Maven, pnpm |
